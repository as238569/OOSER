...
public class ChChessRook extends  ChineseChess{
	...
	public ChessGrid[] getMovableRange(ChessBoard boardInfor) {
		...
		for(int x=thisX; x == thisX ||(x < maxX) && (board[thisY][x].getChess() == null) ; ) //�望蝘餃��斗
		{ 
		...
		for(int x=thisX; x == thisX || (x >= 0) &&( board[thisY][x].getChess() == null) ;)//镼踵蝘餃��斗
		{	
		...
		for(int y=thisY; y == thisY ||(y < maxY) &&(board[y][thisX].getChess() == null) ;)//�蝘餃��斗
		{	
		     y++;
		     if(y < maxY){ 
		...
		for(int y=thisY; y == thisY ||(y >= 0) &&(board[y][thisX].getChess() == null) ;)//�蝘餃��斗	
		{			
			 y--;
			 if(y >= 0){
				 if( board[y][thisX].getChess() != null ){
